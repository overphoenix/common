...
export const isPropertyOwned = (obj: any, field: string) =>
  hasOwnProperty.call(obj, field);
...
export const isEmptyString = (str: any) =>
  typeof str === 'string' && /^\s*$/.test(str); //eslint-disable-line
...
export const isNegativeZero = (val: any) =>
  val === 0 && Number.NEGATIVE_INFINITY === 1 / val;
...
export const isPrefix = (prefix: string, str: string) =>
  getTagSimple(str) === 'str' && str.startsWith(prefix);

// Checks whether `str` ends with `suffix`.
export const isSuffix = (suffix: string, str: string) =>
  getTagSimple(str) === 'str' && str.endsWith(suffix);
...
export const isArrayBuffer = (x: any) =>
  objectProto.toString.call(x) === '[object ArrayBuffer]';
...
export const isPrimitive = (value: any) =>
  ...
  typeof value === 'string' ||
  isBoolean(value) ||
  isSymbol(value); //eslint-disable-line
...
export const isJson = (value: any) =>
  (typeof value === 'string' && value.endsWith('.json')) || isObject(value); //eslint-disable-line

export const isNamespace = (value: any) =>
  isObject(value) && value[asNamespace.SYMBOL] === true;
...
export const isSameType = (value: any, other: any) =>
  typeof value === typeof other && getTagSimple(value) === getTagSimple(other);
...
export const isStream = (value: any) =>
  value !== null && typeof value === 'object' && isFunction(value.pipe);
export const isWritableStream = (value: any) =>
  isStream(value) && typeof value._writableState === 'object';
export const isReadableStream = (value: any) =>
  isStream(value) && typeof value._readableState === 'object';
export const isDuplexStream = (value: any) =>
  isWritableStream(value) && isReadableStream(value);
export const isTransformStream = (value: any) =>
  isStream(value) && typeof value._transformState === 'object';
  ...
  while (i < bytes.length) {
    ...
    if (
      (bytes[i] === 0xe0 &&
        ...
        bytes[i + 1] <= 0xbf &&
        bytes[i + 2] >= 0x80 &&
        bytes[i + 2] <= 0xbf) || // excluding overlongs
...
export const isPathAbsolute =
  process.platform === 'win32' ? isWin32PathAbsolute : isPosixPathAbsolute;
...
export const isAsyncFunction = (fn: any) =>
  fn && toString.call(fn).slice(8, -1) === 'AsyncFunction';
...
export const isArrowFunction = (fn: any) => {
  ...
  return (
    ...
    (isArrowFnWithParensRegex.test(fnStr) ||
      isArrowFnWithoutParensRegex.test(fnStr))
  );
};
...
export const isBinaryPath = (x: any) =>
  binaryExtensions.has(path.extname(x).slice(1).toLowerCase());

export const isIP6 = (ip: any, options?: { exact: boolean }) =>
  regexIP6(options).test(ip);
export const isIP4 = (ip: any, options?: { exact: boolean }) =>
  regexIP4(options).test(ip);
export const isIP = (ip: any, options?: { exact: boolean }) =>
  regexIP(options).test(ip);
...
export const isAsyncEventEmitter = (obj: any) =>
  obj instanceof AsyncEventEmitter;
