...
describe('AsyncEventEmitter', () => {
  ...
  describe('emitReduce()/emitReduceRight()', () => {
    it('listener should be run serially using previous listener return value', async () => {
      ...
      const emitter = new AsyncEventEmitter();
      emitter.on('square', (keys, value) =>
        Promise.resolve([keys.concat(1), value * value]),
      );
      emitter.on('square', (keys, value) =>
        Promise.resolve([keys.concat(2), value * value]),
      );
      emitter.on('square', (keys, value) =>
        Promise.resolve([keys.concat(3), value * value]),
      );

      expect(await emitter.emitReduce('square', [], 2)).toEqual([
        [1, 2, 3],
        256,
      ]);
      expect(await emitter.emitReduceRight('square', [], 2)).toEqual([
        [3, 2, 1],
        256,
      ]);
    });
    ...
    it('if an error occurs, it should throw an error only the first one', async () => {
      const emitter = new AsyncEventEmitter();
      emitter.on('square', (keys, value1) =>
        Promise.resolve([keys.concat(1), value1 * 2]),
      );
