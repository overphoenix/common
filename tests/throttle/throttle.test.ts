  ...
  describe('for promises', () => {
    describe('no intervals', () => {
      ...
      describe('create({ concurrency: n }, fn)', () => {
        it('1 acts as a sequential worker', () => {
          return Promise.all(
            [sentA, sentB, sentC].map(
              throttle.create({ concurrency: 1 }, worker(1)),
            ),
          ).then((res) => {
            assert(
        ...
        it('2 works on two inputs in parallel', () => {
          return Promise.all(
            [sentA, sentB, sentC].map(
              throttle.create({ concurrency: 2 }, worker(2)),
            ),
          ).then((res) => {
            assert(
              res[0] instanceof Processed &&
        ...
        it('3 works on three inputs in parallel', () => {
          return Promise.all(
            [sentA, sentB, sentC].map(
              throttle.create({ concurrency: 3 }, worker(3)),
            ),
          ).then((res) => {
            assert(
              res[0] instanceof Processed &&
      ...
      describe('throttle.create(fn, { concurrency: n })', () => {
        it('1 acts as a sequential worker', () => {
          return Promise.all(
            [sentA, sentB, sentC].map(
              throttle.create(worker(1), { concurrency: 1 }),
            ),
          ).then((res) => {
            assert(
              res[0] instanceof Processed &&
        ...
        it('2 works on two inputs in parallel', () => {
          return Promise.all(
            [sentA, sentB, sentC].map(
              throttle.create(worker(2), { concurrency: 2 }),
            ),
          ).then((res) => {
            assert(
              res[0] instanceof Processed &&
        ...
        it('3 works on three inputs in parallel', () => {
          return Promise.all(
            [sentA, sentB, sentC].map(
              throttle.create(worker(3), { concurrency: 3 }),
            ),
          ).then((res) => {
            assert(
              res[0] instanceof Processed &&
      ...
      describe('drops', () => {
        ...
        it('dropes other calls if concurrency is reached', async () => {
          let i = 0;
          const fn = throttle.create(
            () => ateos.promise.delay(100).then(() => ++i),
            { concurrency: 2, drop: true },
          );

          let [a, b, c, d] = await Promise.all([fn(), fn(), fn(), fn()]);
          expect(a).to.be.equal(1);
          expect(b).to.be.equal(2);
          expect(c).to.be.equal(DROPPED);
          expect(d).to.be.equal(DROPPED);

          [a, b, c, d] = await Promise.all([fn(), fn(), fn(), fn()]);
        ...
        it('should not drop last call if dropLast is false', async () => {
          ...
          const fn = throttle.create(
            () => ateos.promise.delay(100).then(() => ++i),
            { concurrency: 2, drop: true, dropLast: false },
          );
    ...
    describe('intervals', () => {
      ...
      it('should not wait for return: late', async () => {
        ...
        expect(vals).not.to.be.deep.equal(
          range(20).reduce((expected, i) => {
            expected.push(['start', i]);
            expected.push(['end', i]);
            return expected;
      ...
      describe('drops', () => {
        ...
        it('should drop calls if concurrency is reached', async () => {
          ...
          const [a, b, c, d] = await Promise.all([fn(1), fn(2), fn(3), fn(4)]);
