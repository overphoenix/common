...
describe('promisify', () => {
  ...
  describe('multiArgs', () => {
    it('normal', async () => {
      ...
      expect(await promisify(fn, { multiArgs: true })()).toStrictEqual([
        'a',
        'b',
      ]);
    });
    ...
    it('rejection', async () => {
      ...
      expect(
        await promisify(fixture1, { multiArgs: true })().catch(
          (error) => error,
        ),
      ).toStrictEqual(['e', 'a', 'b']);
    });
...
describe('finally', () => {
  ...
  it('returning a rejected promise in the callback rejects the promise', async () => {
    await _finally(Promise.resolve(fixture), () =>
      Promise.reject(fixtureErr),
    ).then(
      () => {
        fail();
      },
  ...
  it('returning a rejected promise in the callback for an already rejected promise changes the rejection reason', async () => {
    await _finally(Promise.reject(new Error('orig err')), () =>
      Promise.reject(fixtureErr),
    ).catch((err) => {
      expect(err).toEqual(fixtureErr);
    });
...
describe('try', () => {
  ...
  it('main', async () => {
    ...
    await expect(async () =>
      _try(() => Promise.reject(fixtureError)),
