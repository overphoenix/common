import { props, try as _try, retry, callbackify, finally as _finally, defer, delay, timeout, nodeify, promisify, promisifyAll } from "../lib/promise";
...
describe("defer", () => {
  ...
  it("should reject the promise", async () => {
    const d = defer();
    d.reject(10);
    expect(await d.promise.then(() => null, (x) => x)).toEqual(10);
  });
});
...
describe("timeout", () => {
  ...
  it("should be rejected by timeout even if rejects by itself synchronously", async () => {
    const q = await timeout(new Promise((resolve, reject) => {
      const t = new Date();
      process.nextTick(() => {
        while (new Date() - t < 200) {
          //
...
describe("promisify", () => {
  ...
  it("should throw if the first argument of the callback truthy", async () => {
    ...
    expect(await f().then(() => null, (x) => x)).toEqual(1);
  });
